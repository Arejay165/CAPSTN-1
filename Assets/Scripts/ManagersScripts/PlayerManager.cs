...
public class PlayerManager : MonoBehaviour
{
    ...
    public void CamShake(GameObject p_object, float p_length, float p_power, float p_rotationMultiplier)
    {


        StartCoroutine(CamShaking(p_object, p_object.transform.position, p_object.transform.rotation, p_power, p_power * p_rotationMultiplier, p_rotationMultiplier, p_power / p_length, p_length, 0.01f));

    }
    public IEnumerator CamShaking(GameObject p_object, Vector3 p_defaultPos, Quaternion p_defaultRotation, float p_shakePower, float p_shakeRotation, float p_rotationMultiplier, float p_shakeFadeTime, float p_length, float p_duration)
    {
        ...
        while (isShaking)
        {
            ...
            else
            {
                ...
                p_object.transform.position = new Vector3(0,0,-10);
                p_object.transform.rotation = Quaternion.Euler(0, 0, 0);
                isShaking = false;
            }
